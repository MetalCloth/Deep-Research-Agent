...
def build_hybrid_retriever(faiss_path=FAISS_PATH, bm25_path=BM25_PATH):
    embeddings = OllamaEmbeddings(model="snowflake-arctic-embed")
    
    try:
        with st.spinner("Loading retrievers from memory..."):
        ...
        st.warning(f"Please Enter Pdf to start")
        st.stop()  

    semantic_retriever = vectorstore.as_retriever(search_kwargs={"k": 4})
    hybrid = EnsembleRetriever(retrievers=[semantic_retriever, bm25], weights=[0.6, 0.4])
    return hybrid
...
def build_multi_source_rag_graph():
    ...
    return graph.compile(checkpointer=st.session_state.checkpoint)
